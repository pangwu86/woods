...
无视语言上的差异,我们尽量提供一致的接口与返回值

### 各个语言的共同之处

一个QWordBuild使用前需要3个步骤:

0. 生成一个使用默认配置项的QWordBuilder
1. 设置配置项
2. 加载查询语言的匹配规则

一般的使用方式是:
    
    qb = new QWordBuilder().setup("配置项").loadRules("规则")

快捷的使用方式是:

    qb = new QWordBuilder("规则")

这时候就可以使用parse方法解析kwd,获得QWord
...
### Golang
	...
	rule,_ :=os.Open('~/myapp/mywords.conf')
	qb := query.QWBuilder(rule)
	qb.Setup(`{sepOr:'| +', sepAnd:',&'}`)
...
> 还有 golang 里面没有char类型, 使用 byte 的话 json化的时候就变天书了, 所以 []char 都以 []string 类型来表示
...
**QWordBuilder**
	...
	// 加载规则, reader可以来自文件(FileReader), 字符串(StringReader)等等
	loadRules(reader)
